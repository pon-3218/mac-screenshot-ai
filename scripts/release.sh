...
case "$mode" in
    ...
    notarized)
        ...
        sparkle_generate_appcast=$(find "$project_dir/.build" -type f -path '*/bin/generate_appcast' -print -quit)
        ...
        print -- "- 自動アップデートに対応" > "$appcast_dir/Capture-Codex-$version-macOS.md"
        print -rn -- "$SPARKLE_EDDSA_PRIVATE_KEY" | \
            "$sparkle_generate_appcast" \
                --ed-key-file - \
                --download-url-prefix "https://github.com/pon-3218/mac-screenshot-ai/releases/download/v$version/" \
                --link "https://github.com/pon-3218/mac-screenshot-ai/releases/tag/v$version" \
                --embed-release-notes \
                --maximum-deltas 0 \
